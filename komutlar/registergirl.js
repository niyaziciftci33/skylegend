...
//---[ Import API ]---\\
const api = new Discord.WebhookClient(webapi.WebhookHost.WebhookOne.ID, webapi.WebhookHost.WebhookOne.TOKEN)
//---[ Import API ]---\\
...
exports.run = async (client, message, args) => {
  ...
  if(!message.member.hasPermission(reg.RoleHost.Admin)) {
    ...
    setTimeout(function() {
      user.roles.add(reg.RoleHost.Girl)
    }, 3000)
    ...
    .setTitle("Hmm, Kayıt İşlemi Gerçekleşti! (Kız - Kadın)")
    .setDescription(`
    ...
    db.add(`StatsGirl_${message.author.id}.${message.guild.id}`, 1)
