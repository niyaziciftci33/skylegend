...
client.on("guildMemberAdd", async member => {
  if(member.guild.id !== "870265878653067335") return;
  
  if(member.guild.id == "870265878653067335") {
   
   const embed = new Discord.MessageEmbed()
    .setDescription(`Hoşgeldin ${member} Kayıt Olmak İçin Kayıt Kanalına İsmini Yaz Ve Yetkilileri Bekle!`)
  client.channels.cache.get(reg.ChannelHost.RegisterJoinMessage)
  .send("<@&870265878653067341>").then(async (i) => {
    i.send(embed)
  });
...
if(member.guild.id == "870265878653067335") {
if(member.bot) {
  member.roles.add("870265878854377506")
}

if(!member.bot) {
let kanal2 = member.guild.channels.cache.get("870265879164776460")

let rol2 = member.guild.roles.cache.get(reg.RoleHost.Unregistered)
...
client.on('emojiDelete', async emoji => {
  const c = emoji.guild.channels.cacheget(db.fetch(`codeminglog_${emoji.guild.id}`));
  if (!c) return;
...
client.on("ready", async () => {
  client.channels.cache.get("884060783204515910").join()
})

/* 
client.on("ready", async () => {
  client.channels.cache.get("883809924965105684").startTyping()
})
